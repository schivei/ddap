...
public class CompressionTests : PageTest
{
    ...
    public async Task BrotliCompression_IsUsed_WhenRequested()
    {
        ...
        var response = await Page.APIRequest.GetAsync($"{DocsBaseUrl}/styles.css", new APIRequestContextOptions
        {
        ...
        if (headers.ContainsKey("content-encoding"))
        {
            ...
            Assert.That(encoding, Is.EqualTo("br").Or.EqualTo("gzip"), 
    ...
    public async Task GzipCompression_IsUsed_WhenBrotliNotSupported()
    {
        ...
        var response = await Page.APIRequest.GetAsync($"{DocsBaseUrl}/theme-toggle.js", new APIRequestContextOptions
        {
            Headers = new Dictionary<string, string>
            {
        ...
        {
            var encoding = headers["content-encoding"];
            Assert.That(encoding, Is.EqualTo("gzip"), 
    ...
    public async Task NoCompression_WhenNotRequested()
    {
        ...
        var response = await Page.APIRequest.GetAsync($"{DocsBaseUrl}/index.html", new APIRequestContextOptions
        {
            Headers = new Dictionary<string, string>
            {
        ...
        {
            var encoding = headers["content-encoding"];
            Assert.That(encoding, Is.Not.EqualTo("br").And.Not.EqualTo("gzip"), 
    ...
    public async Task CSSFiles_AreCompressed()
    {
        ...
        Assert.That(headers.ContainsKey("content-type"), Is.True, "Content-Type header should be present");
        Assert.That(headers["content-type"], Does.Contain("text/css"), "Content-Type should be text/css");
    ...
    public async Task JavaScriptFiles_AreCompressed()
    {
        ...
        var response = await Page.APIRequest.GetAsync($"{DocsBaseUrl}/language-switcher.js", new APIRequestContextOptions
        {
            Headers = new Dictionary<string, string>
            {
                { "Accept-Encoding", "br, gzip" }
    ...
    public async Task HTMLFiles_AreCompressed()
    {
        ...
        Assert.That(headers.ContainsKey("content-type"), Is.True, "Content-Type header should be present");
        Assert.That(headers["content-type"], Does.Contain("text/html"), "Content-Type should be text/html");
    ...
    public async Task Compression_ReducesFileSize()
    {
        ...
        var uncompressedResponse = await Page.APIRequest.GetAsync($"{DocsBaseUrl}/styles.css", new APIRequestContextOptions
        {
            Headers = new Dictionary<string, string>
            {
                { "Accept-Encoding", "identity" }
        ...
        var compressedResponse = await Page.APIRequest.GetAsync($"{DocsBaseUrl}/styles.css", new APIRequestContextOptions
        {
            Headers = new Dictionary<string, string>
            {
                { "Accept-Encoding", "br, gzip" }
        ...
        if (compressedResponse.Headers.ContainsKey("content-encoding"))
        {
            ...
            {
                Assert.That(compressedBody.Length, Is.LessThan(uncompressedBody.Length), 
