...
public class AccessibilityAutoDetectionTests : PageTest
{
    ...
    public async Task Setup()
    {
        ...
        await Page.WaitForFunctionAsync("() => window.ddapTheme !== undefined", 
    ...
    public async Task PrefersReducedMotion_Function_ReturnsBoolean()
    {
        ...
        var hasFunction = await Page.EvaluateAsync<bool>("typeof window.ddapTheme.prefersReducedMotion === 'function'");
    ...
    {
        // Act: Check if function exists and returns boolean
        var hasFunction = await Page.EvaluateAsync<bool>("typeof window.ddapTheme.prefersHighContrast === 'function'");
    ...
    public async Task EnableAutoDetect_Function_Exists()
    {
        ...
        var hasFunction = await Page.EvaluateAsync<bool>("typeof window.ddapTheme.enableAutoDetect === 'function'");
        ...
        var autoDetect = await Page.EvaluateAsync<string?>("localStorage.getItem('ddap-theme-auto-detect')");
        Assert.That(autoDetect, Is.EqualTo("true"), "Auto-detect should be enabled in localStorage");
    }
    ...
    {
        // Act: Check if function exists
        var hasFunction = await Page.EvaluateAsync<bool>("typeof window.ddapTheme.disableAutoDetect === 'function'");
        ...
        // Assert: localStorage should be updated
        var autoDetect = await Page.EvaluateAsync<string?>("localStorage.getItem('ddap-theme-auto-detect')");
        Assert.That(autoDetect, Is.EqualTo("false"), "Auto-detect should be disabled in localStorage");
    }
    ...
    public async Task LightMode_DoesNotHaveHighContrastAttribute()
    {
        ...
        // Assert
        Assert.That(hasHighContrast, Is.False, "Light mode should not have data-high-contrast attribute");
    }
    ...
    public async Task HighContrastMode_LinksAreUnderlined()
    {
        ...
        var textDecoration = await link.EvaluateAsync<string>("el => window.getComputedStyle(el).textDecoration");
        
        // Assert: Links should be underlined in high contrast mode
        Assert.That(textDecoration, Does.Contain("underline"), 
    ...
    public async Task HighContrastMode_FocusIndicators_AreEnhanced()
    {
        ...
        var outlineWidth = await button.EvaluateAsync<string>("el => window.getComputedStyle(el).outlineWidth");
        
        // Assert: Outline should be at least 3px in high contrast mode
        var widthValue = ParseCssPixelValue(outlineWidth);
        Assert.That(widthValue, Is.GreaterThanOrEqualTo(3.0), 
    ...
    {
        if (string.IsNullOrEmpty(cssValue)) return 0;
